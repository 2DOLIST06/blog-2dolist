...
export function BlogPostSeo({ post, path }: { post: { h1?: string; title: string; metaDescription?: string; chapoHtml?: string; heroImageUrl?: string; createdAt?: string; updatedAt?: string }; path: string }) {
  ...
  const breadcrumb = { '@context': 'https://schema.org', '@type': 'BreadcrumbList', itemListElement: [{ '@type': 'ListItem', position: 1, name: 'Accueil', item: siteConfig.baseUrl }, { '@type': 'ListItem', position: 2, name: 'Articles', item: `${siteConfig.baseUrl}/articles` }, { '@type': 'ListItem', position: 3, name: post.title, item: `${siteConfig.baseUrl}${path}` }] };
